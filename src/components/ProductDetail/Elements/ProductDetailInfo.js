...
export default function ProductDetailInfo({ data, onReviewSubmit, hideTab }) {
  ...
  const onAddToCart = (e) => {
    ...
    toast.success("Бүтээгдэхүүн сагслагдлаа");
  };
  const onAddToWishList = (e) => {
    ...
    toast.dismiss();
    if (!product) {
      return toast.success("Бүтээгдэхүүн хүслийн жагсаалт руу орлоо !");
    } else {
    ...
    <div className="product-detail__content">
      ...
      <div className="product-detail__content__footer">
        <ul>
          ...
          <li>
            Бүтээгдэхүүний код: <span>{data.code}</span>
          </li>
          <li>
            Хэмжээ МЛ: <span>{data.point}</span>
          </li>
          <li>
            Нөөц:
            {data.quantity > 0 ? (
              <span className="in-stock"> Байгаа</span>
            ) : (
              <span className="out-stock"> Дууссан</span>
            )}
          </li>
        </ul>
        {data.variation && (
      ...
      {!hideTab && (
        <>
          ...
          <div className="product-detail__content__tab">
            <ProductDetailInfoTab onReviewSubmit={onReviewSubmit} data={data.description} />
          </div>
        </>
      )}
    </div>
